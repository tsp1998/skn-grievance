...
if (empty($User1) || empty($Password1)) {
	...
	$AdminFound = 0;
	// $query = "SELECT PASSWORD FROM `skn_grievance_admin` WHERE user=$User1";
	// $result=mysqli_query($Connection,$query);
	// if($result->num_rows>0){
	// 	echo "<script>alert('login done');</script>";
	// 	}
	$query = 'SELECT * from skn_grievance_admin';
	$result = mysqli_query($Connection, $query);
	if ($result->num_rows > 0) {
	...
	$query = 'SELECT * from skn_grievance_complaints ORDER BY ID DESC';
	$result = mysqli_query($Connection, $query);
	if ($result->num_rows > 0) {
